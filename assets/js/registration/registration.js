...
import { controlPassword } from "../fields/passwordtype.js";
...
window.onload = () => {
    ...
    if (registration_form) {
        ...
        registration_form_email.addEventListener("focus", function () {
            ...
            checkFieldsRegister(registration_form_email, registration_form_login, registration_form_plainPassword, registration_form_phone, registration_form_portrait, registration_form_agreeTerms, registration_form_submit);
        });
        ...
        registration_form_phone.addEventListener("input", function () {
            if (controlPhone(this) === false) {
                let text = "N° de téléphone erroné"; allowPhoto
        ...
        registration_form_submit.addEventListener('click', function (e) {
            ...
            for (let j = 0; j < inputs.length; j++) {
                if (inputs[j].type == 'email' || inputs[j].type == 'text' || inputs[j].type == 'password' || inputs[j].type == 'file' || inputs[j].type == 'checkbox') {
            ...
            for (let i = 0; i < champSuccess.length; i++) {
                ...
                /** type email */
                if (champSuccess[i].type == 'email' && champSuccess[i].value == "" || champSuccess[i].type == 'email' && champSuccess[i].classList.contains('border-red-300')) {
                    alertBorder(champSuccess[i]);
                    compteur++;
                } else if (champSuccess[i].type == 'email' && !champSuccess[i].value == "" && champSuccess[i].classList.contains('border-green-300')) {
                    successBorder(champSuccess[i]);
                    nbBordure++;
                }
                /** type text */

                if (champSuccess[i].type == 'text' && champSuccess[i].value == "" || champSuccess[i].type == 'text' && champSuccess[i].classList.contains('border-red-300')) {
                    alertBorder(champSuccess[i]);
                    compteur++;
                } else if (champSuccess[i].type == 'text' && !champSuccess[i].value == "" && champSuccess[i].classList.contains('border-green-300')) {
                    successBorder(champSuccess[i]);
                    nbBordure++;
                }

                /**type password */
                if (champSuccess[i].type == 'password' && champSuccess[i].value == "" || champSuccess[i].type == 'password' && champSuccess[i].classList.contains('border-red-300')) {
                    alertBorder(champSuccess[i]);
                    compteur++;
                } else if (champSuccess[i].type == 'password' && !champSuccess[i].value == "" && champSuccess[i].classList.contains('border-green-300')) {
                    successBorder(champSuccess[i]);
                    nbBordure++;
                }

                /** type file */
                if(champSuccess[i].type=="file" && !champSuccess[i].classList.contains('border-green-300')){
                    alertBorder(champSuccess[i]);
                    compteur++;
                }else if(champSuccess[i].type=="file" && champSuccess[i].classList.contains('border-green-300')){
                    successBorder(champSuccess[i]);
                    nbBordure++;
                }

                 /** type checkbox */
                if (champSuccess[i].type == "checkbox" && !champSuccess[i].checked) {
                    champSuccess[i].classList.remove('border', 'border-gray-50');
                    champSuccess[i].style.outline = '2px solid #FCA5A5';
                    compteur++;
                } else if (champSuccess[i].type == "checkbox" && champSuccess[i].checked) {
                    champSuccess[i].classList.remove('border', 'border-gray-50');
